...
class WalmartAdapter(PlaywrightRetailerAdapter):
    name = "walmart"

    positive_signals = ["pickup", "in stock", "available"]
    negative_signals = ["out of stock", "sold out", "not available"]
    ...
    def _check_item_with_page(self, page: Page, watch_item: WatchItem) -> StockStatus:
        ...
        LOG.warning("walmart status unknown for %s", watch_item.label)
